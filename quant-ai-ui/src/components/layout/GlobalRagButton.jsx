      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] max-w-[90vw] bg-surface border border-surface-border rounded-lg p-6 z-50 shadow-2xl">
          <div className="flex items-center justify-between mb-4">
            <Dialog.Title className="text-lg font-bold text-foreground">Ask me anything about quant</Dialog.Title>
          ...
          <form onSubmit={handleSubmit} className="flex gap-2 mb-4">
            <input
              ...
              placeholder="Ask a question... e.g. what does an overbought RSI mean"
            ...
            <div className="text-sm text-foreground">
              ...
              {ask.data.evidence?.length > 0 && (
                <details className="text-xs text-muted">
                  <summary className="cursor-pointer">Sources ({ask.data.evidence.length})</summary>
          ...
          {ask.isError && (
            <div className="text-sm text-down">Q&A service is temporarily unavailable. Please try again later.</div>
          )}
        </Dialog.Content>
      </Dialog.Portal>
    </Dialog.Root>
