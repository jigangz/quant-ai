...
class DatasetBuilder:
    ...
    def build(self) -> DatasetOutput:
        ...
        for ticker in self.config.tickers:
            try:
                ...
                # Collect stats
                ticker_stats.append(TickerDataset(
                    ticker=ticker,
                    n_samples=len(df),
                    n_features=len([c for c in df.columns if c not in ["ticker", "date", "label"]]),
                    date_range=(
    ...
    def _time_series_split(
        ...
        val_end_idx = int(n_dates * (
            self.config.split_config.train_ratio + self.config.split_config.val_ratio
        ))
